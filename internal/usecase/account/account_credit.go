...
type AccountCreditRes struct{}

type AccountCreditUseCase usecase.Handler[AccountCredit, *AccountCreditRes]

func NewAccountCreditUseCase(v validation.Service, accountRepo account.Repo, transactionRepo account.TransactionRepo, eventer eventer.Srv) AccountCreditUseCase {
	return func(ctx context.Context, tracer trace.Tracer, req AccountCredit) (*AccountCreditRes, error) {
		ctx = usecase.Push(ctx, tracer, "AccountCredit")
